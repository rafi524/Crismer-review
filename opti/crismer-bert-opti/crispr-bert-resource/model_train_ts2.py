...
def preprocess_bert_data(df, on_col='On', off_col='Off', label_col='Active'):
    Negative, Positive = [], []
...
def eval_model_on_multiple_datasets(model, BATCH_SIZE=256):
    datasets_info = [
        ("Circleseq", "circleseq_all.csv"),
        ("surroseq", "surroseq.csv"),
        ("guideseq", "guideseq.csv"),
        ("ttiss", "ttiss.csv")
    ]
    results = {}
    for name, filename in datasets_info:
        print(f"\n========================================\nEvaluating on {name}...\n========================================")
        path = get_dataset_path(filename)
        test_data = pd.read_csv(path)
        
        test_Neg, test_Pos = preprocess_bert_data(test_data, on_col='On', off_col='Off', label_col='Active')
        Xtest = np.vstack((test_Neg, test_Pos))
        Xtest = Shuffle(Xtest)
        ...
        boot_res = compute_metric_bootstraps(
            curve_data['test_y'], 
            curve_data['pred_y'], 
            curve_data['pred_y_list']
        )
        results[name] = (stats, boot_res)
    return results

if __name__ == '__main__':
    log_file_path = os.path.join(base_dir, "ts2_output.log")
    log_file = open(log_file_path, "w", encoding="utf-8")
    sys.stdout = Tee(sys.stdout, log_file)
    sys.stderr = Tee(sys.stderr, log_file)
    
    data_path = get_dataset_path("changeseq_siteseq.csv")
    df = pd.read_csv(data_path)
    
    Negative, Positive = preprocess_bert_data(df, on_col='On', off_col='Off', label_col='Active')
    ...
    Train_Negative, Validation_Negative = train_test_split(Negative, test_size=0.20, random_state=42)
    Train_Positive, Validation_Positive = train_test_split(Positive, test_size=0.20, random_state=42)
    ...
    results = eval_model_on_multiple_datasets(model, BATCH_SIZE=BATCH_SIZE)
    
    # Save the model weights
    model.save_weights(os.path.join(base_dir, 'crispr_bert_model_ts2.h5'))
    print("Model weights saved to 'crispr_bert_model_ts2.h5'")
    
    # Save metrics summary to ts2_metrics.txt
    metrics_path = os.path.join(base_dir, 'ts2_metrics.txt')
    with open(metrics_path, 'w', encoding='utf-8') as f:
        f.write("=== CRISPR-BERT TS2 Evaluation Metrics ===\n\n")
        for name, (stats, boot_res) in results.items():
            f.write(f"========================================\n")
            ...
            roc_res = boot_res['roc']
            f1_res = boot_res['f1']
            prc_res = boot_res['pr_auc']
