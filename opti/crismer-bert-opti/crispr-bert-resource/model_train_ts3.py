...
# Import Encoder_ts3 (uppercase and range(24)) and model_ts3
from Encoder_ts3 import Encoder, token_dict, BERT_encode, C_RNN_encode
from model_ts3 import build_bert
...
def preprocess_bert_data(df, on_col=0, off_col=1, label_col=2):
    Negative, Positive = [], []
    for idx, row in df.iterrows():
        on_seq = str(row[on_col]).upper().replace('-', '_')
        off_seq = str(row[off_col]).upper().replace('-', '_')
        label_val = int(float(row[label_col]) > 0.0)
...
if __name__ == '__main__':
    log_file_path = os.path.join(base_dir, "ts3_output.log")
    log_file = open(log_file_path, "w", encoding="utf-8")
    sys.stdout = Tee(sys.stdout, log_file)
    sys.stderr = Tee(sys.stderr, log_file)
    
    data_path = get_dataset_path("I1.txt")
    # Read I1.txt without header
    df = pd.read_csv(data_path, header=None)
    
    Negative, Positive = preprocess_bert_data(df, on_col=0, off_col=1, label_col=2)
    ...
    model.save_weights('crispr_bert_model_ts3.h5')
    print("Model weights saved to 'crispr_bert_model_ts3.h5'")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'crispr_bert_curves_ts3.png'))
    print("Saved curve plots to 'crispr_bert_curves_ts3.png'")
    
    np.savez(os.path.join(base_dir, 'crispr_bert_curves_ts3.npz'), 
    ...
    print("🎉 Success! Graphs generated and raw data coordinates saved as 'crispr_bert_curves_ts3.npz'.")
    ...
    # Save metrics summary to ts3_metrics.txt
    metrics_path = os.path.join(base_dir, 'ts3_metrics.txt')
    with open(metrics_path, 'w', encoding='utf-8') as f:
        f.write("=== CRISPR-BERT TS3 Evaluation Metrics ===\n\n")
