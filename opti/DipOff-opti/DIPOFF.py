...
import pandas as pd
import numpy as np
from dipoff_modules import DIPOFF
from utils import *
import os
import pickle
import torch 
...
model_path = os.path.join(pwd, 'models/dipoff_lstm_model.pth')
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Model configuration for RNN_Model_Generic
config = {
    'vocab_size': 0,
    'emb_size': 4,
    'hidden_size': 512,
...
model = RNN_Model_Generic(config, model_type="LSTM")
model = model.to(device)

# Load weights if available
weights_exist = os.path.exists(model_path)
if weights_exist:
    # Use weights_only=False since custom LSTM models are saved, or use load_state_dict safely
    try:
    ...
    print("Please run train_dipoff.py first to train the model, calibrate predictions, and save models/ files.")
...
if os.path.exists(os.path.join(pwd, 'models/bin_weights.pkl')):
    with open(os.path.join(pwd, 'models/bin_weights.pkl'), 'rb') as f:
        bins, weights = pickle.load(f)

if os.path.exists(os.path.join(pwd, 'models/minmax_scaler.pkl')):
    scaler_instance = joblib.load(os.path.join(pwd, 'models/minmax_scaler.pkl'))

dipoff = None
if bins is not None and weights is not None and scaler_instance is not None:
    dipoff = DIPOFF(model=model, bins=bins, weights=weights, ref_genome=GENOME, scaler=scaler_instance)

def check_loaded():
    global dipoff, bins, weights, scaler_instance
    if dipoff is None:
        # Retry loading files just in case they were generated since startup
        if os.path.exists(os.path.join(pwd, 'models/bin_weights.pkl')) and os.path.exists(os.path.join(pwd, 'models/minmax_scaler.pkl')):
            with open(os.path.join(pwd, 'models/bin_weights.pkl'), 'rb') as f:
                bins, weights = pickle.load(f)
            scaler_instance = joblib.load(os.path.join(pwd, 'models/minmax_scaler.pkl'))
            try:
        ...
        if dipoff is None:
            raise RuntimeError("DIPOFF model or calibration files (bin_weights.pkl, minmax_scaler.pkl) are not loaded. "
                               "Please run train_dipoff.py first to train the model and generate these files.")

def cal_score(sgr, tar, ref_genome=GENOME):
    check_loaded()
    dipoff.ref_genome = ref_genome
    return dipoff.single_score_(sgr, tar)

def cal_scores(df_in, On='On', Off='Off', ref_genome=GENOME):
    check_loaded()
    dipoff.ref_genome = ref_genome
    df_in['DIPOFF_Score'] = dipoff.score(df_in)
    return df_in

def cal_spec(df_in, On='On', Off='Off', ref_genome=GENOME):
    check_loaded()
    dipoff.ref_genome = ref_genome
    spec = dipoff.spec_per_sgRNA(data_df=df_in, On=On, Off=Off)
    return spec

def cal_casoffinder_spec(sgr, tar, ref_genome, mm=6, dev='G0'):
    check_loaded()
    dipoff.ref_genome = ref_genome
    dipoff.casoffinder_mm = mm
    spec = dipoff.CasoffinderSpec_(sgr, tar, mm=mm, dev=dev)
    return spec

def opti(tar, ref_genome, threshold=0.77, mm=6, dev='G0'):
    check_loaded()
    dipoff.opti_th = threshold
    dipoff.ref_genome = ref_genome
    csv_df = dipoff.opti(target=tar, mm=mm, dev=dev)
    return csv_df

# Setup the argument parser
def get_parser():
    parser = argparse.ArgumentParser(description="DIPOFF Suite "+__version__)
    module_input = parser.add_argument_group("# DIPOFF modules")
    module_input.add_argument("method", metavar="<method>", type=str, default='score', 
        help="Select a method to calculate the score(s): \n \
        score: DIPOFF-Score of sgRNA-DNA, requires [--sgr, --tar]; \n \
        scores: Batch calculation of DIPOFF-Score, requires [--csv], options [--on_item, --off_item, --out]; \n \
        spec: Calculate DIPOFF-Spec, on-target sequence must be in the first line, requires [--csv], options [--on_item, --off_item]; \n \
        off_spec: Perform Cas-Offinder search and calculate DIPOFF-Spec, requires [--sgr, --tar, --genome], options [--mm, --dev]; \n \
        opti: DIPOFF-Opti optimization by mutation, requires [--tar, --genome], options [--threshold, --mm, --dev, --out]; \n "
        )
    ...
    other_option.add_argument('--threshold', metavar="<float>", type=float, default=0.76, 
        help="The DIPOFF-Score threshold for mutated sgRNAs. (default: 0.76)")

    parser.add_argument('--version', action='version', version='DIPOFF {}'.format(__version__))
...
def main():
    ...
    if args.method == 'score':
        ...
        if args.out == 'default':
            scores.to_csv('DIPOFF-Score_results.csv', index=False)
        else:
            if args.out[-3:] == 'csv':
                out_name = args.out
            else:
        ...
        print('DIPOFF-Score calculation done')
        ...
        if args.out == 'default':
            opti_result.to_csv('DIPOFF-Opti_optimization_results.csv', index=False)
        else:
            if args.out[-3:] == 'csv':
                out_name = args.out
            else:
        ...
        print('DIPOFF-Opti optimization done')
