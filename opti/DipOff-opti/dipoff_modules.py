...
class DIPOFF:
    ...
    def score_bin_(self, y_pred):
        if y_pred.shape[0] != 0:
            y_df = pd.DataFrame(y_pred.reshape(-1, 1), columns=['DIPOFF-Score'])
            return y_df['DIPOFF-Score'].value_counts(bins=self.bins, sort=False).values
    ...
    def opti(self, target, opti_type=None, ref=None, offtar_search=None, mm=6, dev='G0'):
        ...
        cnt_results = self.score_bin_(out_df.loc[:, 'DIPOFF-Score'].values[1:])[-8:]
        ...
        if ref is not None:
            ...
            if out_merge.shape[0] != 0:
                ...
                spec_ref = self.single_spec_(out_merge.loc[:, 'DIPOFF-Score'].values[1:], out_cnt=False)
            else:
                spec_ref = 1.0
            results.append([target, target, 'WT', out_df.loc[:, 'DIPOFF-Score'].values[0]] + list(cnt_results) + [spec, spec_ref])
        else:
            results.append([target, target, 'WT', out_df.loc[:, 'DIPOFF-Score'].values[0]] + list(cnt_results) + [spec])
        ...
        for i in range(opti_nt.shape[0]):
            ...
            if target[pos - 1] != nt:
                ...
                if single_score >= self.opti_th:
                    ...
                    cnt_results = self.score_bin_(out_df.loc[:, 'DIPOFF-Score'].values[1:])[-5:]
                    ...
                    if ref is not None:
                        ...
                        if out_merge.shape[0] != 0:
                            ...
                            spec_ref = 1.0
                    if ref is not None:
                        results.append([new_sgrna, target, '{}{}>{}'.format(target[pos - 1], pos, nt), out_df.loc[:, 'DIPOFF-Score'].values[0]] + list(cnt_results) + [spec, spec_ref])
                    else:
                        results.append([new_sgrna, target, '{}{}>{}'.format(target[pos - 1], pos, nt), out_df.loc[:, 'DIPOFF-Score'].values[0]] + list(cnt_results) + [spec])
        ...
        if ref is not None:
            results_df = pd.DataFrame(results, columns=['sgRNA', 'Target', 'Mutation', 'DIPOFF-Score', '(0.65, 0.7]', '(0.7, 0.75]', 
                                                        '(0.75, 0.8]', '(0.8, 1.0]', 'DIPOFF-Spec', 'ref_Spec'])
        else:
            results_df = pd.DataFrame(results, columns=['sgRNA', 'Target', 'Mutation', 'DIPOFF-Score', '(0.65, 0.7]', '(0.7, 0.75]', 
                                                        '(0.75, 0.8]', '(0.8, 1.0]', 'DIPOFF-Spec'])
        ...
        results_df['delta_Spec'] = results_df.loc[:, 'DIPOFF-Spec'].values - results_df.loc[0, 'DIPOFF-Spec']
